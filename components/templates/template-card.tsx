...
export function TemplateCard({ template, publicMode, compact }: TemplateCardProps) {
  ...
  return (
    <>
      <Card className={cn(
        "flex flex-col h-full hover:bg-[rgba(26,26,26,0.6)] transition-all duration-200",
        compact ? "p-3" : "p-5"
      )}>
        <div
        ...
        >
          ...
          {template.description && (
            <p className={cn(
              "text-[rgba(255,255,255,0.6)] leading-relaxed flex-1",
              compact ? "text-xs line-clamp-1 mb-2" : "text-sm line-clamp-3 mb-4"
            )}>
              {template.description}
            </p>
          )}
