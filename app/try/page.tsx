...
export default function TryPage() {
  ...
  // On mobile, card height fills available viewport (minus nav, heading, dots, padding)
  const [mobileCardHeight, setMobileCardHeight] = useState(340);
  useEffect(() => {
    if (!isMobile) return;
    const calc = () => setMobileCardHeight(Math.max(300, window.innerHeight - 380));
    calc();
          ...
          <div className="flex justify-center gap-0.5 mb-6 md:gap-1 md:mb-8">
            {([
              ...
              >
                {creationPath === key && (
                  <motion.div
                    layoutId="try-creation-path-tab"
